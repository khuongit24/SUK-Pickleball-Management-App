...
class DailyEntryFrame(ttk.Frame):
    ...
    def save_record(self):
        ...
        try:
            ...
            gia = int(''.join(ch for ch in gia_raw if ch.isdigit()) or '0')
            if gia <= 0:
    ...
    def _open_edit_popup_daily(self):
        ...
        def do_save():
            try:
                ...
                new_gia = int(''.join(ch for ch in var_gia.get() if ch.isdigit()) or '0')
    ...
    def refresh_view(self, list_only: bool = False):
        ...
        for idx, r in enumerate(records):
            ...
            tag = 'odd' if idx % 2 else 'even'
            self.tree.insert('', 'end', values=(r.get('san',''), r.get('khung_gio',''), display_gia, r.get('nguoi','')), tags=(tag,))
    ...
    def _perform_bulk_delete(self, ngay: str, win: tk.Toplevel):
        ...
        deleted = 0
        for r in to_del:
            # Sử dụng đúng tham số cho delete_daily_record
...
class MonthlyStatFrame(ttk.Frame):
    ...
    def _open_edit_popup_month(self):
        ...
        def do_save():
            try:
                tong = int(''.join(ch for ch in var_tong.get() if ch.isdigit()) or '0')
                chi_phi = int(''.join(ch for ch in var_cp.get() if ch.isdigit()) or '0')
...
class WaterInputFrame(ttk.Frame):
    ...
    def _open_edit_popup_item(self):
        ...
        try:
            old_price = int(''.join(ch for ch in don_gia_txt if ch.isdigit()))
        except Exception:
            old_price = 0
        ...
        def do_save():
            try:
                new_name = var_name.get().strip()
                price = int(''.join(ch for ch in var_price.get() if ch.isdigit()) or '0')
    ...
    def save_water_item(self):
        ...
        try:
            ...
            price = int(''.join(ch for ch in self.var_item_price.get() if ch.isdigit()) or '0')
            if not name:
                raise ValueError('Tên không được trống')
...
class WaterSalesFrame(ttk.Frame):
    ...
    def _notify_water_input_update(self):
        ...
        if self.water_input_frame:
            try:
                ...
                pass  # Ignore errors if the input frame is not ready
    ...
    def open_delete_sale_dialog(self):
        ...
        try:
            ...
            price = int(''.join(ch for ch in price_str if ch.isdigit()))
        except:
...
💡 Mẹo: Hover chuột để xem tooltip hướng dẫn!
        """
    ...
    def _show_feedback_contact(self):
        ...
        subtitle_label = tk.Label(main_frame, 
                                 text="Chúng tôi luôn lắng nghe ý kiến đóng góp từ bạn!", 
